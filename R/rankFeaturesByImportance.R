...
# train the model
#model1 <- caret::train(tr_status~ gh_num_pr_comments + gh_num_issue_comments + gh_pull_req_num
#                + gh_sloc + 
#                 gh_files_added + gh_files_deleted , data=resultCP, method="lvq", 
#                preProcess="scale", 
#               trControl=control)

#model2 <- caret::train(tr_status~ gh_team_size + gh_num_commit_comments + gh_test_churn + 
#                  gh_src_churn + 
#                  gh_files_modified + gh_tests_added , data=resultCP, method="lvq",
#                preProcess="scale", 
#                trControl=control)

#model3 <- caret::train(tr_status~ gh_tests_deleted + gh_src_files + gh_doc_files + gh_other_files + 
